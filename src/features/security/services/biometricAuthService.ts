import { getUser, getUserId } from '../../auth/domain/authUtils';
...
export class BiometricAuthService {
    ...
    static async isBiometricEnabled(username: string): Promise<boolean> {
        ...
        try {
            ...
            return await securityService.hasBiometricDek(userId);
        } catch (error) {
            logger.warn('failed to check biometric enabled state', { error });
            return false;
        }
    }
