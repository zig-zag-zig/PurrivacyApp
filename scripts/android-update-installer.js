...
function withAndroidUpdateInstaller(config) {
    return withProjectBuildGradle(config, async (config) => {
        const projectRoot = config.modRequest.projectRoot;
        const templateDir = path.join(projectRoot, 'scripts', 'android-update-installer-template');
        ...
        try {
            ...
            for (const f of files) {
                ...
                fs.writeFileSync(
                    dst,
                    source.replace(
                        new RegExp(`^package ${basePackageName.replace(/\./g, '\\.')}(\\.updates)?`, 'm'),
            ...
            mainApp = addKotlinImport(mainApp, `${packageName}.updates.UpdateInstallerPackage`);
            mainApp = addReactPackage(mainApp, 'UpdateInstallerPackage');

            if (mainApp !== originalMainApp) {
                fs.writeFileSync(mainAppPath, mainApp, 'utf8');
            }
        } catch (e) {
            console.error('[android-update-installer-plugin] Failed to configure update installer:', e);
            throw e;
        }
...
module.exports = withAndroidUpdateInstaller;
module.exports.plugin = withAndroidUpdateInstaller;
